...
**Also check for new linters.** On every ESE version bump, review the ese-starter CHANGELOG for any linters added since your previous version. The linter pack (vendored in `ese-starter/scripts/`) evolves independently of the templates: a new drift class discovered in ESE's own self-audit becomes a new linter in the release where it lands. Adopters upgrading via `bash scripts/bootstrap.sh --upgrade` (run from an ese-starter checkout targeting their project) pick up new linters automatically; direct adopters without ese-starter should re-vendor the updated `scripts/lint-*.sh` set on the same update cycle as the standard bump. This keeps the drift-detection surface as broad as ESE's own.

**Use the drift-detection tools.** After every submodule bump, run these adopter-facing tools (all vendored from ese-starter into your project's `scripts/`):
...
1. **Copy starters/ files** to create your initial project files. Required starters:
   ...
   Also vendor the adopter-facing drift-detection linters and workflow tools that apply to your project. These are canonically owned by the `ese-starter` repository per [ADR-2026-04-24](decisions/ADR-2026-04-24-ese-code-canonicalization-ese-starter-as-single-source-for-adopter-facing-executable-code.md): the linter pack (parameterized, portable versions of the linters ESE uses on itself; each catches a specific drift class such as template-instance compliance, FMEA congruence, orphan ADRs, CHANGELOG-to-tag congruence, orphan scripts, README structure, count congruence in prose), and the workflow tools (`new-artifact.sh` to scaffold new artifact instances with placeholders pre-filled, `verify.sh` to run the full linter suite, `upgrade-check.sh` to detect drift after submodule bumps, `catchup.sh` to preview changes between versions). The fastest path is to clone ese-starter and run its bootstrap; adopters managing their own layout can vendor individual scripts into their `scripts/` directory and wire them into CI. See the ese-starter README for the adoption protocol, parameterization guide, and upgrade protocol.
...
| 1.6 | Every documented artifact created from a template contains every required section from that template | Template-compliance linter output (`scripts/lint-template-compliance.sh`, vendored from ese-starter) or compliance review entry | [§4.1](../STANDARDS.md#41-what-must-be-documented) | REQ-4.1-02 |
