...
|-----------|---------|--------|-----|-------------|
| Machine-readable format | Complete (772 REQ-IDs, 320 gates) | Complete | None | - |
...
2. **Requirement validation flow:** CI runs `validate-req-ids.sh` + `lint-req-tags.sh` -> all 772 REQ-IDs validated for uniqueness and tag schema conformance
