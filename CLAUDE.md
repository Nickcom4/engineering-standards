...
Excellence Standards - Engineering (ESE) is a universal, domain-agnostic, stack-agnostic engineering standard. It defines software delivery gates across 9 sections with 736 machine-readable requirements. ESE applies its own standard to itself. Every change must pass 34 CI checks and maintain self-compliance.
...
| `enforcement-spec.yml` | 313 machine-readable gates (auto-generated) |
...
7. **Never cut a release without explicit gate authority instruction.** Release ceremony commits (moving `[Unreleased]` to a versioned heading, bumping `Standard version applied:`, applying a `vX.Y.Z` tag) require the gate authority to explicitly instruct the cut in the same session. Agents may propose a release cut as a recommendation but must not execute one. See [ADR-2026-04-11](docs/decisions/ADR-2026-04-11-release-trigger-policy.md) for the full policy, semver assignment table, and validation criteria.
